...
export function statusTone(value?: string | null): StatusTone {
  ...
  if (["pending", "changes_requested", "changes_needed", "needs_review", "duplicate", "waitlist", "callback", "sending", "offered", "recommended", "invited", "considering", "acceptance_pending", "publicity_pending", "attention", "awaiting_membership"].includes(status)) return "warning";
...
export function statusDescription(value: string | null | undefined, context: StatusContext = "general") {
  ...
  const contextual: Record<StatusContext, Record<string, string>> = {
    ...
    "welcome-email": { sent: "The custom welcome email was delivered for this assignment.", already_sent: "A welcome email was previously delivered; use resend only when needed.", awaiting_membership: "The automatic welcome is held until Google Group membership is verified.", skipped: "The welcome email is intentionally skipped for this assignment.", failed: "The email could not be delivered; the role assignment is still saved.", not_attempted: "No welcome email has been sent for this assignment." },
