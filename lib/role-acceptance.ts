...
export async function ensureRoleAcceptanceRequest(projectId:string, assignmentId:string, actorUserId:string|null, send=true, respectAutoSend=false){
  ...
  if(!request){const customSections=type==="cast"?projectSettings?.cast_sections:projectSettings?.crew_sections;const customIntroduction=type==="cast"?projectSettings?.cast_introduction:projectSettings?.crew_introduction;const customCredits=type==="cast"?projectSettings?.cast_credit_options:projectSettings?.crew_credit_options;const snapshot={name:template.name,type:template.template_type,version:template.version,introduction:customIntroduction||template.introduction,sections:Array.isArray(customSections)?customSections:template.sections,credit_options:Array.isArray(customCredits)&&customCredits.length?customCredits:template.credit_options,project_title:project?.title??"Production",role_name:role?.name??"Role",schedule:{rehearsals:type==="cast"?String(projectSettings?.rehearsal_schedule??""):"",tech_and_dress:String(projectSettings?.tech_schedule??""),performances_and_strike:String(projectSettings?.performance_schedule??"")}};const created=await admin.from("role_acceptance_requests").insert({project_id:projectId,role_assignment_id:assignmentId,person_id:assignment.person_id,template_id:template.id,template_snapshot:snapshot,created_by:actorUserId,expires_at:new Date(Date.now()+days*24*60*60*1000).toISOString()}).select("*").single();if(created.error||!created.data)throw new Error(created.error?.message??"Acceptance request could not be created.");request=created.data;}
  ...
  const attendancePolicy=type==="cast"?"Attendance policy: Three unexcused absences may result in removal from the production. Each unexcused late arrival counts as one unexcused absence.":"Please review every required crew commitment and attendance expectation in your agreement.";const variables={person_name:person.preferred_name||person.full_name,project_title:project?.title??"Siena Theatre",role_name:role?.name??"a production participant",agreement_type:type,attendance_policy:attendancePolicy,role_acceptance_url:url,expires_in:`${days} day${days===1?"":"s"}`};const mailTemplate=await activeEmailTemplate("role_acceptance",projectId,role?.role_group??null);const subject=mailTemplate?renderTemplate(mailTemplate.subject_template,variables):`Role acceptance required: ${variables.project_title}`;const html=mailTemplate?renderTemplate(mailTemplate.body_template,variables,true):`<h1>${variables.project_title}</h1><p>Hello ${variables.person_name},</p><p>You have been selected as <strong>${variables.role_name}</strong>. Please complete the required ${type} agreement.</p><p><strong>${attendancePolicy}</strong></p><p><a href="${url}">Review and Accept My Role</a></p>`;
