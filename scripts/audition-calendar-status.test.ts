...
test("calendar edits are staged for explicit approval or denial", () => {
  ...
  const reviewActions = fs.readFileSync(new URL("../components/calendar-change-review-actions.tsx", import.meta.url), "utf8");
  ...
  assert.match(reviewActions, /button-spinner/);
  ...
  assert.match(page, /approveDisabled=\{ambiguous\}/);
  assert.match(reviewActions, /disabled=\{approveDisabled \|\| busy !== null\}/);
