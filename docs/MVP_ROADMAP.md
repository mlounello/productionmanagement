...
- Implemented: separate cast and technical-crew student agreements based on Siena's recent forms, with exact version snapshots, required acknowledgements, typed attestation, credits, conflicts, cast allergy information, acceptance/decline, and expiration. Project settings include rehearsal, tech/dress, performance, and strike schedules that are frozen into the agreement each student accepts.
- Implemented: cast acceptance invitations and agreements state the attendance policy explicitly and require acknowledgement that three unexcused absences may result in removal and each unexcused late arrival counts as one unexcused absence.
