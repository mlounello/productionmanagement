...
-- Cross-app publicity writes run through one narrow function owned by the
-- migration role. The PM service key never receives blanket access to the
-- Playbill people table.
...
begin
  ...
  insert into app_production_management.audit_log (
    ...
    jsonb_build_object('playbill_person_id', playbill_person_id, 'submission_request_ids', request_ids, 'synced_at', synced_at),
    'Person-approved production publicity snapshot sent to Playbill for editorial review.'
  );
...
-- Supabase custom-schema tables can have different owners depending on which
-- app/migration created them. Pin this bridge to the database migration role
-- and give that role only the relations the validated function touches.
