...
alter table app_production_management.role_assignments
  add column if not exists google_automation_skipped boolean not null default false,
  add column if not exists google_automation_skip_reason text not null default '';
