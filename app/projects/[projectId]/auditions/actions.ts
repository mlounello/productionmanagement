...
export async function createAuditionSessionAction(formData: FormData) {
  ...
  const { data: session, error } = await supabase.from("audition_sessions").insert({
    ...
    ,booking_opens_at: String(formData.get("bookingOpensAt") ?? "") ? easternDate(String(formData.get("bookingOpensAt"))).toISOString() : null
    ,booking_closes_at: String(formData.get("bookingClosesAt") ?? "") ? easternDate(String(formData.get("bookingClosesAt"))).toISOString() : null
    ,reschedule_deadline: String(formData.get("rescheduleDeadline") ?? "") ? easternDate(String(formData.get("rescheduleDeadline"))).toISOString() : null
    ,cancel_deadline: String(formData.get("cancelDeadline") ?? "") ? easternDate(String(formData.get("cancelDeadline"))).toISOString() : null
  }).select("id").single();
