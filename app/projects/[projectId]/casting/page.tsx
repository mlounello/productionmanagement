...
export default async function CastingPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  return <div className="page">
    <header className="page-header"><div><p className="eyebrow">{project.title}</p><h1>Casting & Offers</h1><p className="muted">Prepare the company and review each actor’s proposed role.</p></div><div className="top-actions"><Link className="button secondary" href={`/projects/${projectId}/conflicts`}>Rehearsal conflicts</Link><Link className="button secondary" href={`/projects/${projectId}/onboarding`}>Project agreements & schedules</Link></div></header>
