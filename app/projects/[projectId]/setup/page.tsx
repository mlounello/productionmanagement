...
export default async function ProjectSetupPage({ params, searchParams }: { params: Promise<{ projectId: string }>; searchParams?: Promise<{ step?: string; error?: string }> }) {
  ...
  const reviewSections = readiness.sections.map((readinessSection) => readinessSection.id !== "setup" ? readinessSection : ({ ...readinessSection, items: readinessSection.items.map((item) => ({ ...item, state: "ready" as const, detail: "All guided questions have been reviewed. Finish below to record initial setup as complete." })) }));
  const reviewRequired = reviewSections.flatMap((readinessSection) => readinessSection.items).filter((item) => item.state !== "optional");
  ...
  return <div className="page workspace-page setup-workflow">
    ...
    {requestedStep === "review" ? <><ProjectReadinessChecklist readiness={reviewReadiness}/><section className="panel workspace-section"><div className="section-heading"><div><p className="eyebrow">Final Review</p><h2>{reviewReadiness.attention ? `${reviewReadiness.attention} item${reviewReadiness.attention === 1 ? "" : "s"} still need attention` : "All required checks are ready"}</h2><p className="muted">You may finish initial setup with warnings. The same live checklist stays on Overview and will continue to catch configuration changes.</p></div></div><div className="setup-step-actions"><form action={goToProjectSetupStepAction}><input type="hidden" name="projectId" value={projectId}/><input type="hidden" name="nextStep" value="integrations"/><button className="button secondary" type="submit">Back</button></form><form action={completeProjectSetupAction}><input type="hidden" name="projectId" value={projectId}/><button type="submit">{reviewReadiness.attention ? "Finish setup with warnings" : "Finish project setup"}</button></form></div></section></> : null}
  </div>;
}
