...
export async function GET(request: NextRequest, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  let submissionQuery = supabase.from("audition_submissions").select("id, form_id, answers, private_notes, audition_status, callback_status, casting_status, submitted_at, people(full_name, preferred_name, email, pronouns), audition_slots(starts_at), audition_reviews(rubric, notes, recommendation), audition_files(field_key, file_name, content_type, file_data)").eq("project_id", projectId).is("cancelled_at", null);
  ...
  if (exportType === "roster") {
    ...
    for (const [index,row] of rows.entries()) {
      ...
      if(included.includes("reviewer_notes")){const independent=(row.audition_reviews as Array<Record<string,unknown>>|null)??[];const reviewText=[clean(row.private_notes),...independent.map((review)=>`${clean(review.recommendation)}${review.rubric?` · ${Object.entries(review.rubric as Record<string,unknown>).map(([key,value])=>`${key}: ${value}`).join(", ")}`:""}${review.notes?`\n${clean(review.notes)}`:""}`)].filter(Boolean).join("\n\n");const result=drawAnswer(page,titleFont,bodyFont,y,"Reviewer Notes and Ratings",reviewText);y=result.y;}
