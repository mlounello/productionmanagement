...
export default async function AuditionConfirmationPage({ params, searchParams }: { params: Promise<{ token: string }>; searchParams: Promise<{ access?: string; error?: string; success?: string; warning?: string }> }) {
  ...
  const [{ data },{data:bookingData}] = await Promise.all([supabase.rpc("get_public_audition_form", { form_token: token }),supabase.rpc("get_public_audition_booking_summary",{access_token:query.access})]);
  if (!data) notFound();
  ...
  const bookings=(bookingData??[]) as Array<{field_key:string;auto_assigned:boolean;slot_id:string;starts_at:string;ends_at:string|null;title:string;location:string}>;
  ...
  return <div className="page"><section className="panel"><p className="eyebrow">Audition received</p><h1>Thank you</h1><p>Your form for <strong>{payload.form.title}</strong> has been submitted. Save this page if you need to manage your booking.</p>
    ...
    {bookings.length?<section><h2>Your audition schedule</h2><div className="compact-list">{bookings.map((booking)=><div className="table-row" key={`${booking.field_key}-${booking.slot_id}`}><div><strong>{booking.title}{booking.auto_assigned?" · Automatically assigned":""}</strong><span>{new Date(booking.starts_at).toLocaleString("en-US",{dateStyle:"full",timeStyle:"short",timeZone:"America/New_York"})}{booking.ends_at?`–${new Date(booking.ends_at).toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit",timeZone:"America/New_York"})}`:""} · {booking.location||"Location TBD"}</span></div></div>)}</div></section>:null}
    {payload.form.allow_reschedule&&bookingFields.length<=1 ? <form action={manageAuditionBookingAction} className="form-row"><input type="hidden" name="formToken" value={token} /><input type="hidden" name="accessToken" value={query.access} /><input type="hidden" name="requestedAction" value="reschedule" /><label className="field"><span>Choose a different audition time</span><select name="slotId" required defaultValue=""><option value="">Choose time</option>{slots.map((slot) => { const session = sessions.get(slot.session_id); return <option key={slot.id} value={slot.id}>{session?.title} · {new Date(slot.starts_at).toLocaleString("en-US",{dateStyle:"medium",timeStyle:"short",timeZone:"America/New_York"})} · {session?.location}</option>; })}</select></label><button type="submit">Reschedule</button></form> : null}
