...
export async function submitRoleAcceptanceAction(formData:FormData){
  ...
  const admin=createSupabaseAdminClient(); const {data:request}=await admin.from("role_acceptance_requests").select("id,status,expires_at,role_assignment_id").eq("id",requestId).eq("public_token",token).maybeSingle();
