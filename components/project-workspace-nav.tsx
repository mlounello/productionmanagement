...
export function ProjectWorkspaceNav({ projectId, active }: { projectId: string; active: string }) {
  return (
    <nav className="project-workspace-tabs" aria-label="Project workspace">
      ...
      <Link aria-current={active === "casting" ? "page" : undefined} className={active === "casting" ? "active" : ""} href={`/projects/${projectId}/casting`}>Casting & Offers</Link>
      <Link aria-current={active === "conflicts" ? "page" : undefined} className={active === "conflicts" ? "active" : ""} href={`/projects/${projectId}/conflicts`}>Rehearsal Conflicts</Link>
