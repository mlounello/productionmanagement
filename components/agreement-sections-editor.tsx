...
function RichAgreementField({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (value: string) => void; placeholder: string }) {
  const ref = useRef<HTMLDivElement>(null);
  const [sourceMode,setSourceMode]=useState(false);
  useEffect(() => { if (!sourceMode&&ref.current&&ref.current.innerHTML!==value)ref.current.innerHTML=value; }, [value,sourceMode]);
  function run(command: string, argument?: string) {
    ref.current?.focus();
    document.execCommand(command, false, argument);
    if (ref.current) onChange(ref.current.innerHTML);
  }
  function toggleSource(){if(sourceMode)onChange(sanitizeRichText(value));setSourceMode((current)=>!current);}
  return <div className="field">
    <span>{label}</span>
    <div className="rich-toolbar" role="toolbar" aria-label={`${label} formatting`}>
      <button type="button" className="rich-tool-button" onClick={toggleSource}>{sourceMode?"Hide source":"View source"}</button>
      {!sourceMode?<>
    ...
    {sourceMode?<textarea className="rich-source-editor" rows={8} value={value} onChange={(event)=>onChange(event.target.value)}/>:<div ref={ref} className="rich-editor agreement-rich-editor" contentEditable suppressContentEditableWarning data-placeholder={placeholder} onInput={() => ref.current&&onChange(ref.current.innerHTML)} onBlur={() => ref.current&&onChange(sanitizeRichText(ref.current.innerHTML))}/>}
  </div>;
}
...
export function AgreementSectionsEditor({name,initialSections,label}:{name:string;initialSections:AgreementSection[];label:string}){
  const [sections,setSections]=useState(()=>initialSections.map((section)=>({...section,body:sanitizeRichText(section.body),acknowledgement:sanitizeRichText(section.acknowledgement)})));
  const [sourceMode,setSourceMode]=useState(false);
  ...
  return <div className="agreement-sections-editor">
    ...
    {sourceMode?<div className="field"><span>Section source</span><textarea className="rich-source-editor" rows={24} value={source} onChange={(event)=>setSource(event.target.value)}/>{sourceError?<small className="field-error">{sourceError}</small>:null}</div>:<div className="stacked-form">{sections.map((section,index)=><section className="agreement-editor-section" key={section.key}>
      ...
      <RichAgreementField label="Agreement text" value={section.body} onChange={(body)=>update(index,{body})} placeholder="Write the agreement section…"/>
      <RichAgreementField label="Required acknowledgement" value={section.acknowledgement} onChange={(acknowledgement)=>update(index,{acknowledgement})} placeholder="Write the acknowledgement the student must check…"/>
