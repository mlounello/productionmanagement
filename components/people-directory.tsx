...
export type DirectoryRole = { id: string; name: string; group: string; status: string; statusLabel?: string; projectTitle?: string; guestArtist?: boolean };
...
export type DirectoryPerson = {
  ...
  email: string; vendorNumber: string; phone: string; affiliation: string; personType: string; isSienaEmployee: boolean; status: string;
  headshotUrl: string; managementNotes: string; noteCount: number; projectCount: number; roles: DirectoryRole[]; notes: DirectoryNote[];
  performanceInterests: string[]; technicalInterests: string[]; vocalRange: string; instruments: string; specialSkills: string;
  performanceExperience: string; technicalExperience: string; certificationsTraining: string; danceStyles: string[]; danceExperience: string;
};
...
export function PeopleDirectory({ people, returnTo, projectId, canDeletePeople = false }: { people: DirectoryPerson[]; returnTo: string; projectId?: string; canDeletePeople?: boolean }) {
  ...
  const visible = useMemo(() => people.filter((person) => {
    const haystack = [person.fullName, person.preferredName, person.pronouns, person.email, person.vendorNumber, person.phone, person.affiliation, person.vocalRange, person.instruments, person.specialSkills, person.performanceExperience, person.technicalExperience, person.certificationsTraining, person.danceExperience, ...person.performanceInterests, ...person.technicalInterests, ...person.danceStyles, ...person.roles.flatMap((role) => [role.name, role.group, role.projectTitle ?? ""])].join(" ").toLowerCase();
    ...
    {selected ? <aside className="people-drawer" role="dialog" aria-label={`${selected.fullName} details`} aria-modal="false">
      ...
      <div className="people-drawer-body">
        <form action={updatePersonDirectoryAction} className="stacked-form" key={selected.id}><input type="hidden" name="id" value={selected.id}/><input type="hidden" name="returnTo" value={returnTo}/>
          ...
          <details className="drawer-section" open><summary>Contact & classification</summary><label className="field"><span>Email</span><input type="email" name="email" defaultValue={selected.email}/></label><div className="form-row"><label className="field"><span>Phone</span><input name="phone" defaultValue={selected.phone}/></label><label className="field"><span>Vendor / 90#</span><input name="vendorNumber" defaultValue={selected.vendorNumber}/></label></div><label className="field"><span>Affiliation</span><input name="affiliation" defaultValue={selected.affiliation}/></label><div className="form-row"><label className="field"><span>Person type</span><select name="personType" defaultValue={selected.personType}><option value="person">Person</option><option value="student">Student</option><option value="staff">Staff</option><option value="faculty">Faculty</option><option value="guest_artist">Guest artist</option><option value="vendor_contact">Vendor contact</option><option value="client">Client</option></select></label><label className="field"><span>Status</span><select name="status" defaultValue={selected.status}><option value="active">Active</option><option value="inactive">Inactive</option><option value="archived">Archived</option></select></label></div><label className="checkbox-card"><input name="isSienaEmployee" type="checkbox" defaultChecked={selected.isSienaEmployee}/><span><strong>Siena Employee</strong><small>Makes this person eligible for Theatre Budget access without marking them as a guest artist.</small></span></label></details>
          ...
          <details className="drawer-section"><summary>Interests, skills & experience</summary><label className="field"><span>Technical interests (comma separated)</span><textarea name="technicalInterests" rows={3} defaultValue={selected.technicalInterests.join(", ")}/></label><label className="field"><span>Performance interests (comma separated)</span><textarea name="performanceInterests" rows={3} defaultValue={selected.performanceInterests.join(", ")}/></label><div className="form-row"><label className="field"><span>Vocal range</span><input name="vocalRange" defaultValue={selected.vocalRange}/></label><label className="field"><span>Dance styles (comma separated)</span><input name="danceStyles" defaultValue={selected.danceStyles.join(", ")}/></label></div><label className="field"><span>Instruments and proficiency</span><textarea name="instruments" rows={3} defaultValue={selected.instruments}/></label><label className="field"><span>Special skills</span><textarea name="specialSkills" rows={3} defaultValue={selected.specialSkills}/></label><label className="field"><span>Performance experience</span><textarea name="performanceExperience" rows={4} defaultValue={selected.performanceExperience}/></label><label className="field"><span>Technical experience</span><textarea name="technicalExperience" rows={4} defaultValue={selected.technicalExperience}/></label><label className="field"><span>Dance and movement experience</span><textarea name="danceExperience" rows={3} defaultValue={selected.danceExperience}/></label><label className="field"><span>Certifications and training</span><textarea name="certificationsTraining" rows={3} defaultValue={selected.certificationsTraining}/></label></details>
          <button type="submit">Save person</button>
        </form>
        <section className="drawer-section static"><h3>{projectId ? "Project roles" : "Project history"}</h3><div className="compact-list">{selected.roles.length ? selected.roles.map((role)=><div className="compact-row" key={role.id}><div><strong>{role.name}</strong><span>{role.projectTitle ? `${role.projectTitle} · ` : ""}{label(role.group)}{role.guestArtist ? " · Guest Artist" : ""}</span></div><StatusBadge status={role.status} label={role.statusLabel}/></div>) : <p className="muted">No roles on file.</p>}</div></section>
        ...
        <div className="drawer-footer">{selected.headshotUrl ? <a className="button secondary" href={`/api/people/${selected.id}/headshot/download`}>Download headshot</a> : null}<Link className="button secondary" href={`/people/${selected.id}`}>Open full profile, bio & headshot</Link>{canDeletePeople ? <Link className="button danger" href={`/people/${selected.id}?delete=1#delete-person`}>Delete person…</Link> : null}</div>
      </div>
    </aside> : null}
  </div>;
}
