...
export function AuditionFormBuilder({ projectId, formId, initialSections, initialFields, bookingCategories, action }: {
  projectId: string;
  formId: string;
            ...
            {fields.filter((field) => field.section_key === section.section_key).map((field) => {
              ...
              return (
                <article className="audition-question-editor" key={`${field.field_key}-${globalIndex}`}>
                  ...
                  {field.field_type==="slot_selector"?<fieldset><legend>Booking requirement rules</legend><p className="muted">Use one audition slot selector question for each booking the applicant must make. For example, create one required dance-call question and one required individual-audition question.</p><div className="form-row"><label className="field"><span>Audition block type shown by this question</span><select value={field.settings?.booking_category??"general"} onChange={(event)=>setFields((items)=>items.map((item,index)=>index===globalIndex?{...item,settings:{...item.settings,booking_category:event.target.value}}:item))}>{bookingCategories.map((category)=><option key={category.value} value={category.value}>{category.label}</option>)}</select><small>These choices come directly from the audition blocks created for this project.</small>{bookingCategories.length===0?<small className="setup-warning">Create an audition block first, then return here to select its type.</small>:null}</label><label className="field"><span>Unlock after—and require the same day as</span><select value={field.settings?.same_day_as??""} onChange={(event)=>setFields((items)=>items.map((item,index)=>index===globalIndex?{...item,settings:{...item.settings,same_day_as:event.target.value}}:item))}><option value="">Allow any available day</option>{fields.filter((candidate)=>candidate.field_type==="slot_selector"&&candidate!==field).map((candidate)=><option key={candidate.field_key} value={candidate.field_key}>{candidate.label}</option>)}</select><small>The applicant cannot select this booking until the linked booking is chosen. Available times are then limited to that day.</small></label></div><div className="form-row"><label className="field"><span>Only unlock after this question</span><select value={field.conditional_logic?.field_key??""} onChange={(event)=>setFields((items)=>items.map((item,index)=>index===globalIndex?{...item,conditional_logic:{...item.conditional_logic,field_key:event.target.value}}:item))}><option value="">Always applies</option>{fields.filter((candidate)=>candidate.field_type!=="slot_selector"&&candidate!==field).map((candidate)=><option key={candidate.field_key} value={candidate.field_key}>{candidate.label}</option>)}</select></label><label className="field"><span>When the answer is</span><input value={field.conditional_logic?.value??""} placeholder="Yes or a choice label" onChange={(event)=>setFields((items)=>items.map((item,index)=>index===globalIndex?{...item,conditional_logic:{...item.conditional_logic,value:event.target.value}}:item))}/></label></div></fieldset>:null}
