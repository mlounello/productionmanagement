...
export function HtmlMessageEditor({ name, initialValue, label = "Message" }: { name: string; initialValue: string; label?: string }) {
  ...
  function toggleSource(){if(sourceMode)setValue((current)=>sanitizeRichText(current));setSourceMode((current)=>!current);}
  return <div className="field">
    ...
    {sourceMode
      ? <textarea className="rich-source-editor" rows={14} value={value} onChange={(event) => setValue(event.target.value)} />
