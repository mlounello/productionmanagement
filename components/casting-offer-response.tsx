  ...
  return <form className="panel stacked-form" action={(data) => startTransition(async () => {
    ...
    <fieldset disabled={pending} className="casting-fieldset"><input type="hidden" name="token" value={token}/>
      ...
      {decision === "accepted" ? <>
        ...
        <label className="field"><span>Rehearsal conflicts</span><textarea name="conflicts" maxLength={6000} rows={4}/><small>You can disclose rehearsal conflicts while accepting. Production management will review them.</small></label>
